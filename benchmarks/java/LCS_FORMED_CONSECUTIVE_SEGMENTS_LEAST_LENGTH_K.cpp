...
int f_filled ( int k, string s1, string s2 ) {
  int n = s1 . size ( );
  int m = s2 . size ( );
  int lcs [ n + 1 ] [ m + 1 ];
  int cnt [ n + 1 ] [ m + 1 ];
...
int main(int argc, char** argv) {
    ...
    assert(argc > 1); int i = atol(argv[1]); assert(i < param0.size()); return f_filled(param0[i],param1[i],param2[i]) == f_gold(param0[i],param1[i],param2[i]);;
