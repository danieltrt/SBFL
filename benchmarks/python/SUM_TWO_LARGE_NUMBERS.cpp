...
public : string f_filled ( string str1, string str2 ) {
  ...
  i < n1;
  i ++ ) {
    int sum = ( ( ( unsigned char ) str1 [ i ] ) - 48 ) + ( ( ( unsigned char ) str2 [ i ] ) - 48 ) + carry;
    str += ( char ) ( sum % 10 + 48 );
    carry = ( int ) ( sum / 10 );
  }
  for ( int i = n1;
  i < n2;
  i ++ ) {
    int sum = ( ( ( unsigned char ) str2 [ i ] ) - 48 ) + carry;
    str += ( char ) ( sum % 10 + 48 );
    carry = ( int ) ( sum / 10 );
  }
  if ( ( carry ) && ( carry != 0 ) ) str += ( char ) ( carry + 48 );
  str . resize ( str . size ( ) );
  return str;
}
