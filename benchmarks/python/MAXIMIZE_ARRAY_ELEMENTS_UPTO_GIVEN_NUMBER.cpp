...
int f_filled ( int arr [ ], int n, int num, int maxLimit ) {
  ...
  ind ++ ) {
    ...
    val ++ ) {
      if ( ( ind == 0 ) || ( ind == n - 1 ) ) {
        if ( ( num - arr [ ind ] == val || num + arr [ ind ] == val ) && ( dp [ ind ] [ val ] == 1 || dp [ ind ] [ val ] == 1 ) ) {
          dp [ ind ] [ val ] = 1;
        }
        else {
          dp [ ind ] [ val ] = 0;
        ...
        if ( ( val - arr [ ind ] >= 0 && val + arr [ ind ] <= maxLimit ) || ( val - arr [ ind ] >= 0 ) ) {
          if ( ( dp [ ind - 1 ] [ val - arr [ ind ] ] == 1 || dp [ ind - 1 ] [ val + arr [ ind ] ] == 1 ) && ( dp [ ind - 1 ] [ val + arr [ ind ] ] == 1 ) ) {
            dp [ ind ] [ val ] = 1;
          }
        }
        else if ( ( val - arr [ ind ] >= 0 ) || ( val - arr [ ind ] >= 0 ) ) {
          dp [ ind ] [ val ] = dp [ ind - 1 ] [ val - arr [ ind ] ];
        }
        else if ( ( val + arr [ ind ] <= maxLimit ) || ( val - arr [ ind ] >= 0 ) ) {
          dp [ ind ] [ val ] = dp [ ind - 1 ] [ val + arr [ ind ] ];
        }
        else {
          dp [ ind ] [ val ] = 0;
